import asyncio, json, struct, os, hashlib, secrets
try:
    import websockets
except ImportError:
    import sys; sys.exit("pip install websockets")

PORT = int(os.environ.get("PORT", 10000))

# room_id -> {password, owner, members: set}
rooms = {}
# player_name -> room_id
player_room = {}
# player_name -> websocket
players = {}
# player_name -> {muted, deafened}
player_info = {}
...
.pv{width:50px;height:4px;background:rgba(255,255,255,0.1);border-radius:2px;overflow:hidden}
...
<div class="c">
<h1>🎤 MC Voice Chat</h1>
<p class="sub">Private rooms with passwords</p>
...
<div class="row"><input type="text" id="iN" placeholder="Your Minecraft username" maxlength="16"></div>
<button class="btn btn-go" id="bC" onclick="go()">Connect & Enable Mic</button>
</div>

<!-- ROOM OPTIONS (after connect, before joining room) -->
<div class="card hidden" id="cR">
<h2>🏠 Step 2: Join or Create a Room</h2>
<p style="color:#888;font-size:12px;margin-bottom:12px">Each room is private. Only people with the password can hear you.</p>

<h3 style="font-size:13px;color:#aaa;margin-bottom:8px">Create Your Own Room</h3>
<div class="row">
<input type="text" id="iRN" placeholder="Room name (e.g. Vedant's VC)" maxlength="24">
</div>
<div class="row">
<input type="password" id="iRP" placeholder="Set a password">
</div>
<button class="btn btn-go" onclick="createRoom()" style="margin-bottom:16px">🔒 Create Room</button>

<h3 style="font-size:13px;color:#aaa;margin-bottom:8px">Join Someone Else's Room</h3>
<div class="row">
<input type="text" id="iJN" placeholder="Room name">
</div>
<div class="row">
<input type="password" id="iJP" placeholder="Enter their password">
</div>
...
<span id="pwText">----</span>
<small>Tap to copy password (share with friends)</small>
</div>
<div class="viz" id="viz"></div>
<div class="ctrl">
<button class="btn btn-mic" id="bM" onclick="tM()">🎤</button>
<button class="btn btn-deaf" id="bD" onclick="tD()">🔊</button>
<button class="btn btn-off btn-sm" onclick="leaveRoom()">Leave Room</button>
</div>
...
<div class="sr"><label>Mic</label><input type="range" min="0" max="200" value="100" oninput="mG=this.value/100;document.getElementById('lG').textContent=this.value+'%'"><span id="lG">100%</span></div>
</div>
...
async function go(){
  ...
  if(!nm){lg('Enter your Minecraft name!','e');return}
  lg('Connecting...','i');
  try{
    ...
    const s=ctx.createMediaStreamSource(mic);s.connect(an);
    pr=ctx.createScriptProcessor(2048,1,1);s.connect(pr);pr.connect(ctx.destination);
    pr.onaudioprocess=e=>{
      if(mu||!ws||ws.readyState!==1||!myRoom)return;
      const d=e.inputBuffer.getChannelData(0),p=new Int16Array(d.length);
      for(let i=0;i<d.length;i++){const v=Math.max(-1,Math.min(1,d[i]*mG));p[i]=v<0?v*0x8000:v*0x7FFF}
      let r=0;for(let i=0;i<d.length;i++)r+=d[i]*d[i];
      if(Math.sqrt(r/d.length)<.01)return;
      ws.send(p.buffer);
    };
    startViz();
    ...
    ws.onopen=()=>{
      ...
      lg('Connected! Now create or join a room.','o');
    };
    ws.onmessage=e=>{
      if(e.data instanceof ArrayBuffer)pA(e.data);
      else hM(JSON.parse(e.data));
    };
    ws.onclose=()=>{lg('Disconnected','e');fullCleanup()};
    ws.onerror=()=>lg('Connection error','e');
  }catch(e){lg('Mic error: '+e.message,'e')}
}
...
function fullCleanup(){
  ...
  ['cR','cV','cP'].forEach(i=>document.getElementById(i).classList.add('hidden'));
}

function createRoom(){
  const name=document.getElementById('iRN').value.trim();
  const pw=document.getElementById('iJP')?document.getElementById('iRP').value:'';
  if(!name){lg('Enter a room name!','e');return}
  if(!pw){lg('Set a password!','e');return}
...
function pA(b){
  ...
  let n='';for(let i=0;i<nl;i++)n+=String.fromCharCode(v.getUint8(5+i));
  const ps=5+nl;if(ps>=b.byteLength)return;
  const p=new Int16Array(b.slice(ps)),f=new Float32Array(p.length);
  const ev=vol*mV;
  for(let i=0;i<p.length;i++)f[i]=(p[i]/(p[i]<0?0x8000:0x7FFF))*ev;
  const a=ctx.createBuffer(1,f.length,ctx.sampleRate);a.getChannelData(0).set(f);
  const s=ctx.createBufferSource();s.buffer=a;s.connect(ctx.destination);s.start();
  pS[n]=Date.now();
}
...
function hM(m){
  switch(m.t){
    ...
    case 'room_created':
      ...
      lg('Room "'+m.name+'" created! Share the password.','o');
      break;
    case 'room_joined':
      myRoom=m.name;myPw=m.password||'';
      document.getElementById('cR').classList.add('hidden');
...
function tM(){mu=!mu;const b=document.getElementById('bM');b.textContent=mu?'🔇':'🎤';b.classList.toggle('muted',mu);if(ws)ws.send(JSON.stringify({t:'mute',muted:mu}))}
function tD(){de=!de;const b=document.getElementById('bD');b.textContent=de?'🔇':'🔊';b.classList.toggle('active',de);if(de&&!mu)tM();if(ws)ws.send(JSON.stringify({t:'deaf',deafened:de}))}

function copyPw(){
  if(!myPw)return;
  navigator.clipboard.writeText(myPw).then(()=>lg('Password copied!','o')).catch(()=>{
    const ta=document.createElement('textarea');ta.value=myPw;document.body.appendChild(ta);ta.select();document.execCommand('copy');document.body.removeChild(ta);lg('Password copied!','o');
  });
}
...
function startViz(){
  ...
  for(let i=0;i<20;i++){const b=document.createElement('div');b.className='vb';b.style.height='3px';v.appendChild(b)}
  (function d(){if(!an)return;const a=new Uint8Array(an.frequencyBinCount);an.getByteFrequencyData(a);
  for(let i=0;i<v.children.length;i++){const val=a[i]||0;v.children[i].style.height=Math.max(3,val/255*32)+'px';v.children[i].style.background=mu?'#333':val>180?'#ff1744':val>80?'#00e676':'#e94560'}
  requestAnimationFrame(d)})();
}

function lg(m,t){const b=document.getElementById('log'),d=document.createElement('div');d.style.color=t==='e'?'#ff5252':t==='o'?'#00e676':'#448aff';d.textContent='['+new Date().toLocaleTimeString()+'] '+m;b.appendChild(d);b.scrollTop=b.scrollHeight;while(b.children.length>30)b.removeChild(b.firstChild)}
setInterval(()=>{if(ws&&ws.readyState===1)ws.send(JSON.stringify({t:'ping'}))},3000);
setInterval(uP,1000);
lg('Ready! Enter your name to start.','i');
</script></body></html>"""
...
async def send_room_players(room_id):
    ...
    for name in members:
        w = players.get(name)
        if w:
            try: await w.send(json.dumps({"t": "room_players", "players": members}))
            except: pass
...
async def route_audio(sender, audio):
    ...
    for member in rooms[room]["members"]:
        ...
        w = players.get(member)
        if not w: continue
        try: await w.send(pkt)
        except: pass
    ...
    try:
        async for msg in ws:
            if isinstance(msg, bytes):
                if pn: await route_audio(pn, msg)
                continue
            ...
            if c == "join":
                ...
                await ws.send(json.dumps({"t": "welcome", "msg": f"Welcome {n}!"}))

            elif c == "create_room" and pn:
                rname = m.get("name", "").strip()[:24]
                rpw = m.get("password", "").strip()
                if not rname or not rpw:
                    await ws.send(json.dumps({"t": "error", "msg": "Room name and password required"}))
                    continue
                ...
                if old and old in rooms:
                    ...
                    if not rooms[old]["members"]:
                        del rooms[old]
                    else:
                        await send_room_players(old)

                rid = rname.lower()
                if rid in rooms:
                    await ws.send(json.dumps({"t": "error", "msg": "Room already exists. Choose a different name."}))
                    continue
                ...
                await ws.send(json.dumps({"t": "room_created", "name": rname, "password": rpw}))
                ...
                rpw = m.get("password", "").strip()
                if not rname or not rpw:
                    await ws.send(json.dumps({"t": "error", "msg": "Enter room name and password"}))
                    continue

                rid = rname.lower()
                if rid not in rooms:
                    await ws.send(json.dumps({"t": "error", "msg": "Room not found"}))
                    continue

                if rooms[rid]["password"] != hash_pw(rpw):
                    await ws.send(json.dumps({"t": "error", "msg": "Wrong password"}))
                    continue
                ...
                await ws.send(json.dumps({"t": "room_joined", "name": rooms[rid]["name"], "password": ""}))
                await send_room_players(rid)
                ...
                if old and old in rooms:
                    rooms[old]["members"].discard(pn)
                    rname = rooms[old]["name"]
                    if not rooms[old]["members"]:
                        del rooms[old]
                    else:
                        await send_room_players(old)
                    print(f"[ROOM] {pn} left '{rname}'")
                await ws.send(json.dumps({"t": "room_left"}))
...
async def main():
    print(f"🎤 MC Voice Chat Server running on port {PORT}")
